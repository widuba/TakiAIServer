...
import { transcribe, synthesize, listVoices, isVoiceConfigured, isRealtimeConfigured, defaultAgentId, getConversationToken } from "./src/voice.js";
...
/* ============================================================================
 * Voice Phase 2 — realtime conversation (ElevenLabs Agents Platform).
 *
 * The device connects DIRECTLY to ElevenLabs over WebRTC (sub-second latency,
 * natural barge-in) using a short-lived token minted here. Crucially, the agent's
 * "LLM" is configured in the ElevenLabs dashboard as a Custom LLM pointing at the
 * shim below — so every spoken reply is still generated by OUR Gemini brain +
 * persona, not ElevenLabs' own model. That keeps Phase 2 as capable as text.
 * ==========================================================================*/

// Device asks for a fresh WebRTC token to open a realtime session with our agent.
app.get("/api/voice/token", async (req, res) => {
  if (!isRealtimeConfigured()) {
    res.status(503).json({ error: "realtime voice not configured (set ELEVENLABS_API_KEY + ELEVENLABS_AGENT_ID)" });
    return;
  }
  const agentId = typeof req.query?.agentId === "string" ? req.query.agentId : undefined;
  const token = await getConversationToken(agentId);
  if (!token) { res.status(502).json({ error: "could not mint conversation token" }); return; }
  res.json({ token, agentId: agentId || defaultAgentId() });
});

// Turn a string / OpenAI content-parts array into plain text.
function llmContentToText(content: any): string {
...
// Our brain, over an OpenAI ChatCompletions message list → a single reply string.
async function brainReplyForMessages(messages: any[], timeZone?: string): Promise<string> {
  const convo = (Array.isArray(messages) ? messages : []).filter(
    (m) => m && (m.role === "user" || m.role === "assistant") && llmContentToText(m.content)
  );
  // Last user turn is the query; everything before it is history/context.
  let lastUserIdx = -1;
  for (let i = convo.length - 1; i >= 0; i--) { if (convo[i].role === "user") { lastUserIdx = i; break; } }
  if (lastUserIdx < 0) return "";
  const query = llmContentToText(convo[lastUserIdx].content);
  const history = convo.slice(0, lastUserIdx).map((m) => ({
    role: m.role as "user" | "assistant",
    text: llmContentToText(m.content),
    createdAt: new Date().toISOString()
  }));
  const contextStr = JSON.stringify({ followUpContext: "", activeChatTitle: "Voice", chatMessages: history.slice(-40) });
  // Realtime MVP: default persona (device profile isn't in the OpenAI request yet)
  // and unmetered (per-turn credit metering for realtime is Phase 2.5).
  const userProfile = parseUserPersona(undefined, undefined);
  const state = buildConversationState(query, contextStr, undefined, timeZone, [], userProfile);
  try {
    const plan = await withTimeout(planAssistantResponse(state), 30000, "Realtime plan");
    const finalized = finalizeResponse(plan, state);
    let text = finalized.spokenText || "";
    if (text && (finalized.action || finalized.memory?.pendingClarification)) {
      text = await styleInCharacter(text, state.userProfile);
    }
    if (!text) text = await getGeneralAnswer(state);
    return text || "Sorry, I didn't catch that.";
  } catch (error) {
    console.error("Realtime brain error:", error);
    try { return await getGeneralAnswer(state); } catch { return "I had trouble with that — say it once more?"; }
  }
}

// Write one OpenAI-style streaming chunk.
function sseChunk(res: any, id: string, created: number, model: string, delta: any, finish: string | null) {
  res.write(`data: ${JSON.stringify({ id, object: "chat.completion.chunk", created, model, choices: [{ index: 0, delta, finish_reason: finish }] })}\n\n`);
}

// Custom-LLM endpoint the ElevenLabs agent calls (dashboard "Server URL" =
// https://<host>/v1). ElevenLabs sends an OpenAI ChatCompletions request and
// expects OpenAI-compatible output (SSE stream when stream=true).
app.post("/v1/chat/completions", async (req, res) => {
  // Optional shared-secret guard: if VOICE_LLM_SECRET is set on Render, configure
  // the same value as the agent's Custom-LLM API key so only it can call this.
  const secret = process.env.VOICE_LLM_SECRET || "";
  if (secret) {
    const auth = String(req.headers["authorization"] || "");
    if (auth !== `Bearer ${secret}` && auth !== secret) { res.status(401).json({ error: "unauthorized" }); return; }
  }
  const body = req.body || {};
  const model = typeof body.model === "string" ? body.model : "taki-brain";
  const stream = body.stream !== false; // ElevenLabs streams by default
  const tz = typeof body?.user_metadata?.timeZone === "string" ? body.user_metadata.timeZone : undefined;

  const text = await brainReplyForMessages(body.messages, tz);
  ...
  sseChunk(res, id, created, model, { role: "assistant" }, null);
  // Stream sentence-by-sentence so ElevenLabs can start speaking sooner.
  const parts = text.match(/[^.!?]+[.!?]+(\s|$)|[^.!?]+$/g) || [text];
  for (const part of parts) { if (part) sseChunk(res, id, created, model, { content: part }, null); }
  sseChunk(res, id, created, model, {}, "stop");
  res.write("data: [DONE]\n\n");
  res.end();
});
