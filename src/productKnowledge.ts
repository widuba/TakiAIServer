...
export function productAnswerFor(message: string, context: ProductAnswerContext = {}): string | null {
  ...
  if (/\b(?:taki )?models?\b|\bwhat (?:model|version) are you\b|\b(?:swift|reasoning) model\b/.test(m)) {
    ...
    return `You're using ${selected.name}, which is ${selected.detail.toLowerCase()}. Tap Model in text chat or voice mode to expand the Faster-to-Smarter control; it switches immediately as you drag. Swift is generally faster and cheaper across a variety of everyday questions; Taki 2.1 balances speed, depth, and typical credit use; Reasoning is generally more thorough and expensive. These are overall tendencies, not guarantees for every request. Every model can research the web and uses Taki's reliable action planner. Credits reflect the model tokens and paid tools actually used, so a web lookup can cost more on any model.`;
  }
