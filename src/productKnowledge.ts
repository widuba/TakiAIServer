...
export function productAnswerFor(message: string, context: ProductAnswerContext = {}): string | null {
  ...
  if (/\b(?:taki )?models?\b|\bwhat (?:model|version) are you\b|\b(?:swift|reasoning) model\b/.test(m)) {
    ...
    return `You're using ${selected.name}, which is ${selected.detail.toLowerCase()}. Tap Model in text chat or voice mode to expand the Faster-to-Smarter control; it switches immediately as you drag. Swift is fastest and uses the fewest credits but has a higher chance of inaccurate or outdated information; Taki 2.1 balances speed, credit use, accuracy, and current information; Reasoning takes longer and uses the most credits but is most likely to be accurate, thorough, and up to date. No model is guaranteed to be correct. Device actions always use Taki's reliable action planner.`;
  }
